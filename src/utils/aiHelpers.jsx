...
const client = new OpenAI({
  ...
  dangerouslyAllowBrowser: true // MODIFIED: Using A4F provider prefix
});

// console.log(API_URL);
export const getChatbotResponse = async (message, previousContext = []) => {
  try {
    // Format the previous context into the API format
    const formattedContext = previousContext.map(item => ({
      role: item.role || 'user',
      content: item.content
    }));

    const completion = await client.chat.completions.create({
    model: "provider-5/gpt-4o", // MODIFIED: Using A4F provider prefix
    messages: [
      ...formattedContext,
